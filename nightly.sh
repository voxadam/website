...
#
# Hello! This is a script that installs Dgraph nightly build
# into your PATH (which may require password authorization).
# Use it like this:
#
#	$ curl https://nightly.dgraph.io | bash
#
...
printf $RESET
	...
	if [ "$dgraph" == "$dgraphsum" ] && [ "$dgraphloader" == "$dgraphloadersum" ]; then
		print_good "You already have the latest version of Dgraph nightly installed."
	else
	...
	if [ "$assets" == "$assetsSum" ]; then
		...
		curl -L --progress-bar https://github.com/dgraph-io/dgraph/releases/download/${tag}/$assetsFile -o /tmp/$assetsFile;
	...
	print_instruction "Please visit https://docs.dgraph.io/master/get-started for further instructions on usage."
}
